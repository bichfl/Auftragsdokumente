...
function initUI() {
    ...
    if (istMobil()) {
        ...
        if (sel && sel.options.length > 0) {
            sel.value = localStorage.getItem('selectedMobileProfile') || sel.options.value;
        }
        ...
        if (sel && sel.options.length > 0) {
            sel.value = localStorage.getItem('selectedDesktopProfile') || sel.options.value;
        }
...
function closeDichtungswechselOverlay() {
    ...
    if (daSelect) { daSelect.innerHTML = '<option value="">Dichtung außen</option>'; daSelect.disabled = true; }
    if (diSelect) { diSelect.innerHTML = '<option value="">Dichtung innen</option>'; diSelect.disabled = true; }
    if (eiSelect) { eiSelect.innerHTML = '<option value="">Ersatzdichtung innen</option>'; eiSelect.disabled = true; }
...
function showQualitaetsmeldung() {
    ...
    <div class="global-qm-container">
      <div class="qm-header-row">
        <input type="text" id="qmPosInputField" placeholder="Pos.Nr.">
        <div class="qm-toggle-container">
          <span id="lblHinweis" data-text="Hinweis" class="qm-label">Hinweis*</span>
          <label class="qm-switch">
            <input type="checkbox" id="qmPriorityToggle">
            <span class="qm-slider"></span>
          </label>
          <span id="lblHandlungsbedarf" data-text="Handlungsbedarf" class="qm-label">Handlungsbedarf**</span>
        </div>
      </div>
      <div class="input-group">
        <textarea id="qmDescriptionField" rows="8" placeholder="Problembeschreibung"></textarea>
      </div>
      <button class="qm-send-btn" onclick="sendQualitaetsmeldung()">Senden</button>
      <span class="qm-hint-text">* Auftrag läuft - Weiterarbeit möglich</span>
      <span class="qm-hint-text">** Auftrag steht - Weiterarbeit NICHT möglich</span>
    </div>
  `;
